...
namespace DoomLib
{
	...
	typedef int ( *RecvFunc)( char* buff, int *numRecv );
    
    // typedef int ( *SendFunc)( const char* buff, DWORD size, sockaddr_in *target, int toNode );
    typedef int ( *SendFunc)( const char* buff, int size, sf::IpAddress target, int toNode );
    ...
    void						SetCurrentDifficulty( const std::string& name );
    const std::string &			GetCurrentDifficulty();
